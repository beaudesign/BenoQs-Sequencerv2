...
Every non-trivial behavioral constant in `octocore` is cited to a page of the real *Octopus Reference Manual, CE OS v5.30* (genoQs Machines, Stuttgart 2009), which ships in this repo at `reference/manual/`. Where the manual is genuinely ambiguous or a piece of behavior hasn't been implemented yet, that's recorded honestly in `tests/conformance/AMBIGUITIES.md` rather than guessed at silently. 45 tests, all green.

The wenge design vision the crate was built against — a hardware-accurate panel renderer with physically-simulated materials and motion, and a "rehearsal rooms" concept tying acoustic and visual environments together — lives in `SPEC.md` and `docs/`. None of the rendering/room half exists yet; only the sequencer core and its C ABI boundary do.
