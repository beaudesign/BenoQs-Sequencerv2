...
Every non-trivial behavioral constant in `octocore` is cited to a page of the real *Octopus Reference Manual, CE OS v5.30* (genoQs Machines, Stuttgart 2009), which ships in this repo at `reference/manual/`. Where the manual is genuinely ambiguous or a piece of behavior hasn't been implemented yet, that's recorded honestly in `tests/conformance/AMBIGUITIES.md` rather than guessed at silently. 56 unit tests and 3 conformance fixtures, all green.
